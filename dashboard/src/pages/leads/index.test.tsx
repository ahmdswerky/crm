...
function boardResponse(stats: Record<string, number> = {}, pending: unknown[] = []) {
  return {
    stats,
    columns: {
      pending: { data: pending, total: pending.length, next_cursor: null, has_more: false },
      contacted: { data: [], total: 0, next_cursor: null, has_more: false },
      qualified: { data: [], total: 0, next_cursor: null, has_more: false },
      unqualified: { data: [], total: 0, next_cursor: null, has_more: false },
    },
  }
}
...
describe("LeadsKanbanPage", () => {
  ...
  it("hides zero-valued statuses from the progress labels", async () => {
    const fetchSpy = vi.fn(async (_input: RequestInfo | URL) => new Response(JSON.stringify(boardResponse({ pending_count: 2, contacted_count: 0, unqualified_count: 1, qualified_count: 0 })), { status: 200, headers: { "Content-Type": "application/json" } }))
    ...
    const boardRequests = fetchSpy.mock.calls.filter(([input]) => String(input).includes("/v1/leads/board"))
  ...
  it("renders the assigned agent avatar returned with a lead", async () => {
    ...
    const lead = { id: 4, name: "Nadia Saleh", email: "nadia@example.com", phone: "+201001234568", status: "pending", city: "Cairo", address: null, company_name: null, source: null, assigned_agent_id: 4, assigned_agent: { id: 4, name: "Omar Saad", username: "omar", email: "omar@example.com", avatar } }
    vi.stubGlobal("fetch", vi.fn(async () => new Response(
      JSON.stringify(boardResponse({ pending_count: 1, contacted_count: 0, unqualified_count: 0, qualified_count: 0 }, [lead])),
  ...
  it("opens the lead create modal and submits a pending lead", async () => {
    ...
    const fetchSpy = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      return new Response(JSON.stringify(boardResponse()), { status: 200, headers: { "Content-Type": "application/json" } })
    })
    vi.stubGlobal("fetch", fetchSpy)
    ...
    expect(boardRequests.every(([, init]) => !init?.method || init.method === "GET")).toBe(true)
  })
