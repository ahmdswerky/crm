...
import { LeadDetailsPage, LeadsPage } from "./details"
...
describe("LeadsPage", () => {
  ...
  it("shows each assigned agent without their email and uses the URL filter", async () => {
    ...
    const agentLink = (await screen.findAllByText("Mona Hassan")).map((node) => node.closest("a")).find((link): link is HTMLAnchorElement => link !== null)
    expect(agentLink).toHaveAttribute("href", "/agents/12")
