...
export function CustomerCard({ data, isLoading }: DataCardProps<Customer>) {
  ...
  return <section className="min-w-0 overflow-hidden rounded-3xl bg-card"><CardHeader title="Customers" to="/leads" /><Table><TableHeader className="bg-muted/45"><TableRow><TableHead className="px-5">Name</TableHead><TableHead>Company</TableHead><TableHead className="pe-5">Position</TableHead></TableRow></TableHeader><TableBody>{data.map((customer) => <TableRow key={customer.id} className="hover:bg-muted/40"><TableCell className="px-5 py-3.5 font-medium whitespace-normal">{customer.name}</TableCell><TableCell className="py-3.5 whitespace-normal"><span className="flex items-center gap-2">{customer.accountLogo ? <img src={customer.accountLogo} alt="" className="size-6 rounded-md bg-background object-contain p-1" /> : <span aria-hidden="true" className="grid size-6 place-items-center rounded-md bg-muted text-[0.625rem] text-muted-foreground">—</span>}{customer.company}</span></TableCell><TableCell className="pe-5 py-3.5 whitespace-normal text-muted-foreground">{customer.position}</TableCell></TableRow>)}</TableBody></Table></section>
}
