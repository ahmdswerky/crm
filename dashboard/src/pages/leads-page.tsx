...
import { ArrowLeft, Building2, Eye, Loader2, Mail, Pencil, Phone, Plus, RefreshCw, Save, Search, Trash2, UserRoundPlus, X } from "lucide-react"
import { z } from "zod"
...
type AgentOption = { id: number; name: string; username: string }
type LeadEnvelope = { lead: Lead }
type ContactCreateEnvelope = { contact: { id?: number } }

const contactConversionSchema = z.object({
  account_id: z.number().int().min(1, "Choose an account."),
  title: z.string(),
})

type ContactConversionValues = z.infer<typeof contactConversionSchema>
...
async function loadAccountOptions(query: string, page: number, signal: AbortSignal): Promise<SearchableResourcePage> {
  ...
  const options = collectionValues(body).flatMap((value): SearchableResourceOption[] => {
    if (typeof value !== "object" || value === null) return []
    const record = value as Record<string, unknown>
    const id = typeof record.id === "number" ? record.id : Number(record.id)
    const name = typeof record.name === "string" ? record.name : ""
    if (!Number.isInteger(id) || id < 1 || !name) return []
    return [{ id, label: name, description: typeof record.industry === "string" ? record.industry : undefined }]
  })
  const meta = typeof body === "object" && body !== null && typeof (body as Record<string, unknown>).meta === "object" && (body as Record<string, unknown>).meta !== null ? (body as Record<string, unknown>).meta as Record<string, unknown> : {}
  return {
    options,
  ...
  useEffect(() => {
    ...
    void apiJson<Paginated<User>>(`${API_BASE_URL}/v1/users`, { signal: controller.signal })
      .then(async (firstPage) => {
        ...
        setAgentOptions(users.filter((user): user is User & { id: number } => user.id !== undefined).map((user) => ({ id: user.id, name: user.name, username: user.username })))
      })
...
function ConvertToContactDialog({ lead }: { lead: LeadListRecord }) {
  const [open, setOpen] = useState(false)
  const [error, setError] = useState("")
  const navigate = useNavigate()
  const form = useForm<ContactConversionValues>({
    resolver: zodResolver(contactConversionSchema),
    defaultValues: { account_id: 0, title: "" },
  })
  const { register, setValue, watch, formState: { errors, isSubmitting } } = form

  const openDialog = () => {
    form.reset({ account_id: 0, title: "" })
    setError("")
    setOpen(true)
  }
  ...
  const submit = form.handleSubmit(async (values) => {
    ...
    try {
      ...
      const payload: { lead_id: number; account_id: number; title?: string } = { lead_id: lead.id, account_id: values.account_id }
      if (title) payload.title = title
      const result = await apiJson<ContactCreateEnvelope>(`${API_BASE_URL}/v1/contacts`, {
        method: "POST",
        body: JSON.stringify(payload),
      })
      if (!result.contact?.id) throw new Error("The contact response did not include the created contact.")
      setOpen(false)
      navigate(`/contacts/${result.contact.id}`)
    } catch (caught) {
      if (caught instanceof ApiError) Object.entries(caught.fields).forEach(([field, messages]) => form.setError(field as keyof ContactConversionValues, { message: messages[0] }))
      setError(caught instanceof Error ? caught.message : "Unable to convert this lead to a contact.")
    }
  })

  return <>
    <ActionTooltip label="Convert to contact"><Button type="button" variant="ghost" size="icon" aria-label="Convert to contact" onClick={(event) => { event.preventDefault(); event.stopPropagation(); openDialog() }}><UserRoundPlus /></Button></ActionTooltip>
    <Dialog open={open} onOpenChange={(nextOpen) => { if (!isSubmitting) { setOpen(nextOpen); if (!nextOpen) setError("") } }}>
      {open && <DialogContent>
        <DialogHeader>
          <DialogTitle>Convert {lead.name} to contact</DialogTitle>
          <DialogDescription>Review the contact details before creating this contact from the lead.</DialogDescription>
        </DialogHeader>
        <form onSubmit={submit} className="space-y-4">
          <FieldGroup className="gap-4">
            <SearchableResourcePicker
              id={`convert-contact-account-${lead.id}`}
              label="Account"
              required
              icon={<Building2 className="size-3.5" aria-hidden="true" />}
              value={watch("account_id")}
              onChange={(value) => setValue("account_id", value, { shouldDirty: true, shouldValidate: true })}
              error={errors.account_id?.message}
              loadOptions={loadAccountOptions}
              placeholder="Choose an account"
              searchPlaceholder="Search accounts…"
              loadingLabel="Searching accounts…"
              emptyLabel="No accounts found."
              noResultsLabel="No accounts match your search."
              renderOption={(option) => <span className="min-w-0"><span className="block truncate font-medium">{option.label}</span>{option.description && <span className="block truncate text-xs text-muted-foreground">{option.description}</span>}</span>}
              renderSelectedOption={(option) => <span className="truncate">{option.label}</span>}
            />
            <Field>
              <InputGroup className="h-auto! overflow-hidden">
                <InputGroupAddon align="block-start" className="bg-muted dark:bg-muted"><InputGroupText id={`convert-contact-title-label-${lead.id}`}>Title</InputGroupText></InputGroupAddon>
                <InputGroupInput id={`convert-contact-title-${lead.id}`} aria-labelledby={`convert-contact-title-label-${lead.id}`} {...register("title")} />
              </InputGroup>
              <FieldError>{errors.title?.message}</FieldError>
            </Field>
          </FieldGroup>
          <section className="border border-border bg-muted/20 p-4">
            <div className="flex items-center gap-3"><PersonAvatar name={lead.name} size="sm" /><p className="font-semibold">{lead.name}</p></div>
          </section>
          {error && <p role="alert" className="text-sm text-destructive">{error}</p>}
          <DialogFooter>
            <Button type="submit" className="w-full" disabled={isSubmitting}>{isSubmitting ? <><Loader2 className="size-3.5 animate-spin" aria-hidden="true" />Converting…</> : "Convert"}</Button>
          </DialogFooter>
        </form>
      </DialogContent>}
    </Dialog>
...
function LeadForm({ create = false, form, title, saving, onCancel, onSubmit, formId }: { create?: boolean; form: ReturnType<typeof useForm<LeadFormValues>>; title: string; saving: boolean; onCancel: () => void; onSubmit: () => void; formId?: string }) {
  const { register, formState: { errors }, setValue, watch } = form
  const nameField = <FormField compact={create} label="Name" required error={errors.name?.message}><Input className={create ? "h-8 rounded-none border-0 px-2.5 focus-visible:border-0 focus-visible:ring-0 focus-visible:ring-offset-0" : undefined} {...register("name")} /></FormField>
  const phoneField = <FormField compact={create} label="Phone" required error={errors.phone?.message}><Input className={create ? "h-8 rounded-none border-0 px-2.5 focus-visible:border-0 focus-visible:ring-0 focus-visible:ring-offset-0" : undefined} type="tel" {...register("phone")} /></FormField>
  const emailField = <FormField compact={create} label="Email" required error={errors.email?.message}><Input className={create ? "h-8 rounded-none border-0 px-2.5 focus-visible:border-0 focus-visible:ring-0 focus-visible:ring-offset-0" : undefined} type="email" {...register("email")} /></FormField>
  const statusField = <LeadSelectField compact={create} id="lead-status" label="Status" required error={errors.status?.message} value={watch("status")} onValueChange={(value) => setValue("status", value as LeadFormValues["status"], { shouldValidate: true })}>{statuses.map((status) => <SelectItem key={status} value={status}><StatusOption status={status} /></SelectItem>)}</LeadSelectField>
  const cityField = <FormField compact={create} label="City" required error={errors.city?.message}><Input className={create ? "h-8 rounded-none border-0 px-2.5 focus-visible:border-0 focus-visible:ring-0 focus-visible:ring-offset-0" : undefined} {...register("city")} /></FormField>
  const sourceField = <LeadSelectField compact={create} id="lead-source" label="Source" value={watch("source") || "none"} onValueChange={(value) => setValue("source", value === "none" ? "" : value as LeadFormValues["source"], { shouldValidate: true })} placeholder="No recorded source"><SelectItem value="none">No recorded source</SelectItem>{sources.map((source) => <SelectItem key={source} value={source}><SourceOption source={source} /></SelectItem>)}</LeadSelectField>
  const companyField = <FormField compact={create} label="Company" error={errors.company_name?.message}><Input className={create ? "h-8 rounded-none border-0 px-2.5 focus-visible:border-0 focus-visible:ring-0 focus-visible:ring-offset-0" : undefined} {...register("company_name")} /></FormField>
  const addressField = <FormField compact={create} label="Address" error={errors.address?.message} className={create ? "sm:col-span-2" : "sm:col-span-2"}><Textarea className={create ? "min-h-20 rounded-none border-0 px-2.5 py-2 focus-visible:border-0 focus-visible:ring-0 focus-visible:ring-offset-0" : undefined} {...register("address")} /></FormField>
...
function FormField({ label, required, error, className, compact = false, children }: { label: string; required?: boolean; error?: string; className?: string; compact?: boolean; children: React.ReactNode }) {
  const id = useId()
  const labelId = `${id}-label`
  const control = isValidElement<{ id?: string; "aria-labelledby"?: string; "data-slot"?: string }>(children) ? cloneElement(children, { id, ...(compact ? { "aria-labelledby": labelId, "data-slot": "input-group-control" } : {}) }) : children
...
function LeadSelectField({ id, label, required, error, compact = false, value, onValueChange, placeholder, children }: { id: string; label: string; required?: boolean; error?: string; compact?: boolean; value: string; onValueChange: (value: string) => void; placeholder?: string; children: ReactNode }) {
  const labelId = `${id}-label`
  const select = <Select value={value} onValueChange={onValueChange}><SelectTrigger id={id} aria-labelledby={compact ? labelId : undefined} className={compact ? "h-8 w-full rounded-none border-0 bg-transparent px-2.5 shadow-none focus-visible:border-0 focus-visible:ring-0" : "w-full"}><SelectValue placeholder={placeholder} /></SelectTrigger><SelectContent>{children}</SelectContent></Select>
...
function AgentOption({ agent }: { agent: AgentOption }) {
  return <span className="flex min-w-0 items-center gap-2"><PersonAvatar name={agent.name} size="sm" /><span className="min-w-0"><span className="block truncate">{agent.name}</span></span></span>
}
