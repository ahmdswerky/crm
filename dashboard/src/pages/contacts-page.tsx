...
import { ArrowLeft, Building2, CalendarDays, Eye, Mail, Pencil, Phone, Plus, RefreshCw, Save, Search, Trash2, UserRound, X } from "lucide-react"
import { z } from "zod"
...
type ContactCreateRequest = ContactPaths["/contacts"]["post"]["requestBody"]["content"]["application/json"]
type ContactUpdateRequest = ContactPaths["/contacts/{id}"]["post"]["requestBody"]["content"]["application/json"]
type ContactBasePayload = Pick<ContactCreateRequest, "name" | "phone" | "account_id"> & Partial<Pick<ContactCreateRequest, "title" | "email">>
type ContactCreatePayload = ContactBasePayload & { lead_id: number }
...
type PendingLead = Omit<MarketingComponents["schemas"]["Lead"], "id"> & { id: number }
type LeadListResponse = MarketingPaths["/"]["get"]["responses"][200]["content"]["application/json"]
type ContactEnvelope = { contact: Contact }

const contactPermission = { view: "contact.view", create: "contact.create", edit: "contact.edit", delete: "contact.delete" } as const
...
const contactSchema = z.object({
  ...
  lead_id: z.number().int().min(0),
})

type ContactFormValues = z.infer<typeof contactSchema>
type ContactListBody = { data?: unknown[]; contacts?: unknown[]; meta?: { current_page?: number; last_page?: number } }
const emptyValues: ContactFormValues = { name: "", title: "", email: "", phone: "", account_id: 0, lead_id: 0 }
...
async function loadPendingLeadOptions(query: string, page: number, signal: AbortSignal): Promise<SearchableResourcePage> {
  const params = new URLSearchParams({ status: "pending", per_page: "20", page: String(page) })
  if (query.trim()) params.set("q", query.trim())
  const body = await apiJson<LeadListResponse>(`${API_BASE_URL}/v1/leads?${params}`, { signal })
  const options = body.data.filter((lead): lead is PendingLead => lead.id !== undefined).map((lead) => ({ id: lead.id, label: lead.name, description: `${lead.email} · ${lead.city}`, data: { name: lead.name, email: lead.email, phone: lead.phone } }))
...
function valuesFromContact(contact: ContactRecord): ContactFormValues { return { name: contact.name, title: contact.title ?? "", email: contact.email ?? "", phone: contact.phone, account_id: contact.account?.id ?? contact.account_id ?? 0, lead_id: contact.lead?.id ?? 0 } }
...
function toCreatePayload(values: ContactFormValues): ContactCreatePayload { return { ...toBasePayload(values), lead_id: values.lead_id } }
...
export function ContactsPage() {
  ...
  useEffect(() => {
    ...
    if (mode === "create") {
      navigate(`/contacts/create${returnSearch ? `?return=${encodeURIComponent(returnSearch)}` : ""}`, { replace: true })
      return
    }
  ...
  const canCreate = can(contactPermission.create); const canEdit = can(contactPermission.edit); const canDelete = can(contactPermission.delete)
  const createPath = `/contacts/create${returnSearch ? `?return=${encodeURIComponent(returnSearch)}` : ""}`
  const previewOpen = Boolean(selectedId && mode !== "create" && mode !== "edit")
  return <div className="space-y-6 p-6 lg:p-8">
    <div className="flex flex-wrap items-end justify-between gap-4 border-b border-border pb-6"><div><p className="text-xs font-medium text-muted-foreground">CRM / People</p><h1 className="mt-2 text-2xl font-semibold tracking-tight">Contacts</h1><p className="mt-1 max-w-2xl text-sm text-muted-foreground">Keep the people behind every account visible, connected, and easy to reach.</p></div><div className="flex gap-2"><Button variant="outline" size="sm" onClick={() => { setNotice(""); void loadContacts() }} disabled={loading}><RefreshCw className="me-2 size-3.5" />Refresh</Button>{canCreate && <Button asChild size="sm"><Link to={createPath}><Plus className="me-2 size-3.5" />New contact</Link></Button>}</div></div>
    ...
    <section data-testid="contacts-table-surface" className="min-w-0 border border-border bg-card">
      <Table><TableHeader><TableRow><TableHead>Name</TableHead><TableHead>Account</TableHead><TableHead>Reach</TableHead><TableHead className="w-48 min-w-48 max-w-48 text-end">Actions</TableHead></TableRow></TableHeader><TableBody>
        ...
        }) : <TableRow><TableCell colSpan={4} className="h-32 text-center text-muted-foreground">{hasFilters ? <>No contacts match the current filters. <Button variant="link" size="sm" className="ms-1" onClick={clearFilters}>Clear filters</Button></> : <div><p>No contacts found.</p>{canCreate && <Button asChild variant="link" size="sm" className="mt-1"><Link to={createPath}>Create a contact</Link></Button>}</div>}</TableCell></TableRow>}
...
export function ContactDetailsPage({ create = false }: { create?: boolean } = {}) {
  const { can } = useAuth()
  ...
  const editing = !create && searchParams.get("mode") === "edit" && can(contactPermission.edit)
  ...
  const createContact = form.handleSubmit(async (values) => {
    if (!values.lead_id) {
      form.setError("lead_id", { message: "Choose a pending lead." })
      return
    }
    setError("")
    try {
      await apiJson<Record<string, never>>(`${API_BASE_URL}/v1/contacts`, { method: "POST", body: JSON.stringify(toCreatePayload(values)) })
      navigate(indexHref, { replace: true })
    } catch (caught) {
      if (caught instanceof ApiError) Object.entries(caught.fields).forEach(([field, messages]) => form.setError(field as keyof ContactFormValues, { message: messages[0] }))
      setError(caught instanceof Error ? caught.message : "Unable to create this contact.")
    }
  })
  ...
  if (create && !can(contactPermission.create)) return <ErrorState kind="forbidden" title="Contact creation is restricted" description="You do not have permission to create contacts." actionLabel="Return to contacts" actionTo="/contacts" />
  if (!create && !can(contactPermission.view)) return <ForbiddenContacts />
  if (create) return <main className="mx-auto max-w-[100rem] space-y-6 p-6 pb-24 lg:p-8"><header className="flex flex-wrap items-end justify-between gap-6"><div className="min-w-0"><Button asChild variant="ghost" size="sm" className="-ms-2"><Link to={indexHref}><ArrowLeft className="me-2 size-3.5" />Back to contacts</Link></Button><h1 className="mt-4 text-2xl font-semibold tracking-tight">New Contact</h1></div><div className="flex shrink-0 items-center gap-2"><Button type="button" variant="outline" onClick={() => navigate(indexHref)}>Cancel</Button><Button type="submit" form="contact-create-form" disabled={form.formState.isSubmitting}>{form.formState.isSubmitting ? "Creating…" : "Create"}</Button></div></header>{error && <div role="alert" className="border border-destructive/30 bg-destructive/5 p-3 text-sm text-destructive">{error}</div>}<ContactDetailsEditor create form={form} formId="contact-create-form" hideToolbar saving={form.formState.isSubmitting} accountLink={null} onCancel={() => navigate(indexHref)} onSubmit={createContact} /></main>
...
function BlockStartField({ inputId, label, required, error, icon, children, className }: { inputId: string; label: string; required?: boolean; error?: string; icon?: ReactNode; children: ReactNode; className?: string }) {
...
function ContactDetailsEditor({ create = false, form, saving, accountLink, onCancel, onSubmit, hideToolbar = false, formId }: { create?: boolean; form: ReturnType<typeof useForm<ContactFormValues>>; saving: boolean; accountLink: ReactNode; onCancel: () => void; onSubmit: () => void; hideToolbar?: boolean; formId?: string }) {
  ...
  function selectLead(leadId: number, option?: SearchableResourceOption) {
    setValue("lead_id", leadId, { shouldDirty: true, shouldValidate: true })
    if (!isRecord(option?.data)) return
    for (const field of ["name", "email", "phone"] as const) {
      const value = stringValue(option.data[field])
      if (value !== undefined) setValue(field, value, { shouldDirty: true, shouldValidate: true })
    }
  }
  return <form id={formId} onSubmit={onSubmit} className={hideToolbar ? "w-full" : "w-full border-t border-border pt-5"}>
    {!hideToolbar && <div className="sticky top-0 z-10 -mt-5 border-b border-border bg-background py-4"><div className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-xs font-medium text-muted-foreground">{create ? "Create contact" : "Edit contact"}</p><h2 className="mt-1 text-xl font-semibold tracking-tight">Contact information</h2></div><div className="flex gap-2"><Button type="button" variant="outline" onClick={onCancel}>Cancel</Button><Button type="submit" disabled={saving}>{saving ? "Saving…" : <><Save className="me-2 size-3.5" />{create ? "Create contact" : "Save changes"}</>}</Button></div></div></div>}
    <FieldGroup className={`${hideToolbar ? "mt-0" : "mt-6"} grid w-full gap-4 sm:grid-cols-2`}>
      {create && <SearchableResourcePicker id="contact-details-lead" label="Lead" required icon={<UserRound className="size-3.5" aria-hidden="true" />} value={watch("lead_id")} onChange={selectLead} error={errors.lead_id?.message} loadOptions={loadPendingLeadOptions} placeholder="Choose a pending lead" searchPlaceholder="Search pending leads…" loadingLabel="Searching pending leads…" emptyLabel="No pending leads found." noResultsLabel="No pending leads match your search." description="Only leads with pending status are available." className="sm:col-span-2" />}
      {create && <SearchableResourcePicker id="contact-details-account" label="Account" required icon={<Building2 className="size-3.5" aria-hidden="true" />} value={watch("account_id")} onChange={(accountId) => setValue("account_id", accountId, { shouldDirty: true, shouldValidate: true })} error={errors.account_id?.message} loadOptions={loadAccountOptions} placeholder="Choose an account" searchPlaceholder="Search accounts…" loadingLabel="Searching accounts…" emptyLabel="No accounts found." noResultsLabel="No accounts match your search." className="sm:col-span-2" />}
      ...
      <BlockStartField inputId="contact-details-phone" label="Phone" required error={errors.phone?.message}><InputGroupInput id="contact-details-phone" aria-labelledby="contact-details-phone-label" type="tel" {...register("phone")} className={inputClassName} /></BlockStartField>
