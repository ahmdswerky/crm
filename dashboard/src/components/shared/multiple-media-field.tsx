...
export function StagedMultipleMediaField({ disabled = false, maxFiles = 10, onFilesChange }: StagedMultipleMediaFieldProps) {
  return <MediaDropzone className="sm:col-span-2" multiple maxFiles={maxFiles} disabled={disabled} emptyTitle="Drop property images here" emptyDescription={`JPEG, PNG, WebP, or AVIF · up to ${maxFiles} images, 10 MB each`} ariaLabel="Property images" uploadMode="deferred" queueLayout="vertical" onFilesChange={onFilesChange} />
}
...
export function MultipleMediaField({ ownerType, ownerId, collection, label, description, disabled = false, maxFilesPerUpload = 10, onChange }: MultipleMediaFieldProps) {
  ...
  return <section className="space-y-4 border border-border bg-card p-4">
    ...
    <MediaDropzone multiple maxFiles={maxFilesPerUpload} disabled={disabled || busy} emptyTitle="Drop listing images here" emptyDescription={`JPEG, PNG, WebP, or AVIF · up to ${maxFilesPerUpload} images, 10 MB each`} queueLayout="vertical" onUpload={upload} />
    ...
    </div> : !loading && <div className="flex items-center gap-3 border border-dashed border-border bg-muted/20 p-4"><ImageIcon className="size-5 text-muted-foreground" aria-hidden="true" /><div><p className="text-sm font-medium">Your gallery is empty</p><p className="text-xs text-muted-foreground">Stage images above, then upload them together.</p></div></div>}
  </section>
}
